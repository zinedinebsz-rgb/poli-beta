    ...
    <div className="min-h-screen surface-0 flex items-center justify-center p-4">
      <div className="w-full max-w-md animate-fadeIn">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            {mode === 'register' && (
              <div>
                ...
                <input type="text" value={name} onChange={(e) => setName(e.target.value)}
                  className="input-base w-full" placeholder="Ton nom" required />
              </div>
            )}
            ...
            <div>
              ...
              <input type="email" value={email} onChange={(e) => setEmail(e.target.value)}
                className="input-base w-full" placeholder="toi@email.com" required />
            </div>
            ...
            {mode !== 'reset' && (
              <div>
                ...
                <input type="password" value={password} onChange={(e) => setPassword(e.target.value)}
                  className="input-base w-full" placeholder="••••••••" required minLength={6} />
              </div>
            )}
            ...
            {message && (
              <div className="bg-emerald-500/10 border border-emerald-500/20 rounded-lg px-3 py-2 text-emerald-400 text-sm">{message}</div>
            )}
        ...
        <p className="text-center text-zinc-600 text-xs mt-8">
          {"POLI Beta \u00b7 Gestion de patrimoine \u00e9thique"}
        </p>
      </div>
    </div>
  );
